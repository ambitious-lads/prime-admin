...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen bg-white overflow-hidden flex flex-col">
      ...
      {/* Main Hero Container */}
      <main className="flex-grow flex items-center px-4 sm:px-6 lg:px-4 max-w-7xl mx-auto w-full py-10 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 items-center w-full">
          
          {/* Left Column: Heading, description, CTA buttons, and social proof */}
          <div className="lg:col-span-6 flex flex-col justify-center space-y-8 animate-fade-in-up">

            {/* Main Headline */}
            <div className="space-y-4">
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black tracking-tight font-accent text-ink leading-[1.15]">
                Practice Smarter to Ace UAT
              </h1>
              
              {/* Supporting Text */}
              <p className="text-base sm:text-lg text-muted max-w-xl leading-relaxed font-sans font-medium">
                All-in-one platform to practice smarter, take realistic mock exams, and track your progress — specifically designed to help you ace the Addis Ababa University Entrance Exam (AAU UAT).
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 pt-6 border-t border-line/40">
              <div className="flex -space-x-3">
                {avatars.map((avatar, idx) => (
                  <div
                    key={idx}
                    className="relative w-10 h-10 rounded-full border-2 border-white overflow-hidden shadow-md hover:scale-110 hover:z-10 transition-all duration-250 cursor-pointer"
                  >
                    <Image
                      src={avatar.src}
                      alt={avatar.alt}
        ...
        {/* Creative Primary Color Layered Paint Wave / Painting at the Bottom */}
        <div className="absolute bottom-0 left-0 right-0 w-full overflow-hidden leading-[0] -z-10 select-none pointer-events-none">
          {/* Background soft paint layer */}
          <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-full h-[70px] text-brand/10 fill-current">
            <path d="M0,20 C300,90 550,-10 900,80 C1050,110 1150,50 1200,30 L1200,120 L0,120 Z"></path>
          </svg>
          {/* Foreground solid brand paint layer */}
          <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-full h-[55px] text-brand fill-current -mt-6">
            <path d="M0,45 C150,85 380,10 650,75 C900,110 1080,30 1200,50 L1200,120 L0,120 Z"></path>
          </svg>
        </div>
      </div>
    );
